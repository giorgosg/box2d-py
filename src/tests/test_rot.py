...
def test_rot_multiplication():
    rot1 = Rot.FromAngle(math.pi/4)
    rot2 = Rot.FromAngle(math.pi/2)
    combined = rot1 * rot2
    assert combined.angle_radians == aprx(3 * math.pi / 4)

def test_rot_equality():
    rot1 = Rot.FromAngle(math.pi/2)
    rot2 = Rot.FromAngle(math.pi/2)
    assert rot1 == rot2

    rot3 = Rot.FromAngle(math.pi/4)
    assert rot1 != rot3

def test_rot_string_representation():
    rot = Rot.FromAngle(math.pi/2)
    assert str(rot) == f"Rot(c={rot.c:.6f}, s={rot.s:.6f})"

def test_rot_repr():
    rot = Rot.FromAngle(math.pi/2)
    assert repr(rot).startswith("Rot.FromAngle(")
...
def test_rot_from_b2rot():
    class Mockb2Rot:
        def __init__(self, c, s):
            self.c = c
            self.s = s
    b2_rot = Mockb2Rot(math.cos(math.pi/4), math.sin(math.pi/4))
    rot = Rot.from_b2Rot(b2_rot)
    assert rot.angle_radians == aprx(math.pi/4)
