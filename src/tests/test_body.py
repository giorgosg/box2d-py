...
def test_dynamic_body():
    world = World()
    body = world.new_body().dynamic().position(2.0, 3.0).build()
...
def test_sleep_threshold_behavior():
    world = World()
    body = (
        world.new_body().dynamic().sleep_threshold(0.1).linear_velocity(0.2, 0).build()
    )
...
def test_angular_velocity_via_builder():
    world = World()
    body = world.new_body().dynamic().angular_velocity(3.0).build()
